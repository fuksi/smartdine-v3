...
export function ProductModal({ product, locationId, isOpen, onClose }: ProductModalProps) {
  const [quantity, setQuantity] = useState(1)
  const [selectedOptions, setSelectedOptions] = useState<Record<string, string[]>>({})
  const { addItem, setLocation } = useCartStore()

  const handleOptionChange = (optionId: string, valueId: string, isMultiSelect: boolean) => {
    ...
    Object.entries(selectedOptions).forEach(([optionId, valueIds]) => {
      ...
      if (option) {
        valueIds.forEach(valueId => {
          ...
          if (value) {
            total += Number(value.priceModifier)
          }
        })
      }
    })
  ...
  const handleAddToCart = () => {
    ...
    const cartOptions = Object.entries(selectedOptions).flatMap(([optionId, valueIds]) => {
      const option = product.options.find(o => o.id === optionId)
                    ...
                    {option.optionValues.map(value => (
                      <div key={value.id} className="flex items-center space-x-2">
                        <Checkbox.Root
                          ...
                          onCheckedChange={() => handleOptionChange(option.id, value.id, true)}
